...
const PROFILE_PATH = '/tmp/profile-5d-10f-vs-5d-10f.cpuprofile'
const TOP_N = 25

function buildSideState(faction: FactionKey): SideStateData {
  const selections = createDefaultUnitSelections()
  selections.DREADNOUGHT = { count: 5, upgraded: false }
  selections.FIGHTER = { count: 10, upgraded: false }
...
async function main() {
  const attacker = buildSideState(FACTION)
  const defender = buildSideState(FACTION)
