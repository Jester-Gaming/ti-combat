...
describe('APOLLO', () => {
  it.fails(
    'stamps Hero subtype on the pre-galvanized target at combat construction',
    () => {
      const t = combatTest({
        mode: 'SPACE',
        attacker: {
      ...
      const plainGalvanized = cruisers.filter(
        u =>
          u.subtypes?.includes('Galvanized') && !u.subtypes?.includes('Hero'),
      )
  ...
  it.fails(
    'fires when the Hero unit is destroyed — rolls dice at hero combat value',
    () => {
