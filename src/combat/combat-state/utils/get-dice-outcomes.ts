 ...
 * groups (Cartesian product of per-group distributions).
 *
 * Unlike getCombinedDiceDistribution (which convolves totals), this preserves
 * per-group hit counts so abilities can distinguish which group produced hits.
 */
export function getDiceOutcomes(diceGroups: DiceGroup[]): DiceOutcome[] {
  ...
  for (const group of diceGroups) {
    const dist: DiceRollOutcome[] = getDiceDistribution(group)
